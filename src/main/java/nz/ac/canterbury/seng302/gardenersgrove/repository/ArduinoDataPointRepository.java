...
public interface ArduinoDataPointRepository extends CrudRepository<ArduinoDataPoint, Long> {
    ...
    @Query("SELECT a FROM ArduinoDataPoint a WHERE a.garden.id=?1 ORDER BY PARSEDATETIME(a.time, 'yyyy-MM-dd HH:mm:ss.SSSSSS') DESC")
    ...
    @Query("SELECT a FROM ArduinoDataPoint a WHERE a.garden.id=?1 and a.time >= ?2 and a.time <= ?3 ORDER BY PARSEDATETIME(a.time, 'yyyy-MM-dd HH:mm:ss.SSSSSS') ASC")
