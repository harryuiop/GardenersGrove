...
public class ErrorChecker {
  ...
  public Map<String, String> gardenFormErrors(
    ...
    if (!formValuesValidator.checkDescription(gardenDescription)
        || !formValuesValidator.checkContainsText(gardenDescription)) {
   ...
   * @param errorName     Name of the error field to add to the map (to be used to
   ...
   * @return A HashMap<String, String> of the errors that have occurred based on
   *         the outcome of the error checks.
   */
  ...
  /**
   * Takes the user's first name and checks that there is a first name, it is 64
  ...
  /**
   * Checks if the given last name, if given, is not unintentionally blank,
   * contains 64 characters or fewer, and it
  ...
  /**
   * Given the users email checks whether the email is present, in valid form and
   ...
   *                    new one.
   * @param userService The current user service to check whether the new email is
   *                    in use.
   * @return A Map<String, String> of errors containing <error model name, error
   *         message>.
   */
  ...
  public Map<String, String> emailErrorsResetPassword(String email) {
    Map<String, String> errors = new HashMap<>();
    if (formValuesValidator.checkBlank(email) || !emailIsValid(email))
      errors.put("emailError", "Email address must be in the form ‘jane@doe.nz");
    return errors;
  }

  /**
   * Given the user's birthdate checks whether the data is present, in valid
  ...
  /**
   * Receives the inputs for the profile (from edit directly but is also used by
   * register below),
   ...
   * @param userService The userService instance used to query the database for
   *                    checking whether an account
  ...
  /**
   * Checks the provided information on the register page. If any fields are
   * invalid, it returns
   ...
   * @param validDate       Whether the date is valid or not.
   * @param dateOfBirth     The date of birth string.
   * @param userService     The userService instance used to query the database
   *                        for checking whether an account
   *                        exists with the given information.
   * @return A HashMap<String, String> of the errors that have occurred based on
   *         the outcome of the error checks.
   */
  ...
  /**
   * Checks the provided information from a user on the login in page. If the
   * email is valid and the password
   * for that account is correct, then no errors are returned, otherwise, it
   * returns a map of errors and their
   * descriptions.
   *
  ...
  /**
   * Creates a hash map of all the errors that it encounters from
   * FormValuesValidator checks
   *
   ...
   *                            Editing password needs old password, resetting
   *                            does not.
   *
  ...
  public Map<String, String> editPasswordFormErrors(String oldPassword,
    ...
    } else if (newPassword.contains(user.getFirstName()) || newPassword.contains(user.getLastName())
        || newPassword.contains(user.getEmail())) {
    ...
    } else if (retypeNewPassword.contains(user.getFirstName()) || retypeNewPassword.contains(user.getLastName())
        || retypeNewPassword.contains(user.getEmail())) {
